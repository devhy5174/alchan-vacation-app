...
export function calcResultStats(plan: VacationPlan, completion: CompletionRecord): ResultStats {
  ...
  for (const date of dates) {
    ...
    const tasks = weeklySchedule[dow] ?? [];
    const dateStr = toDateStr(date);
    const arr = completion[dateStr] ?? [];

    const dayTotal = tasks.length;
    const dayDone = arr.filter(Boolean).length;
  ...
  const stickerCount = Math.floor(achievementRate / 10);
